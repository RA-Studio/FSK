...
$this->setFrameMode(true);?>
    <div class="card mfp-hide" id="card-example" style="display: block !important;">
        <div class="p-relative">
            <div class="h1 card__title"><?=$arResult['SECTION']['PATH'][0]['~NAME']?>, <?=$arResult['NAME']?> м<sup>2</sup></div>
            <div class="card__body js-tab-wrapper">
                <div class="card__col-1">
                    <div class="card__tabs-wrap">
                        <div class="card__tabs js-tabs">
                            <?foreach ($arResult['PROPERTIES']['image']['VALUE'] as $key => $img):?>
                                <?
                                    $type = $arResult['PROPERTIES']['image']['DESCRIPTION'][$key];
                                    $title = '';
                                    switch ($type) {
                                        case "plan": $title = 'Планировка'; break;
                                        case "floor_plan": $title = 'На этаже'; break;
                                        case "section": $title = 'Схема корпусов'; break;
                                        case "decoration": $title = 'Отделка'; break;
                                    }
                                ?>
                                <button class="js-tab<?= $key === 0 ? " js-tab--active" : "" ?>" type="button"><?=$title?></button>
                            <?endforeach?>
                        </div>
                    </div>
                    <div class="tooltip">
                    ...
                    <?foreach ($arResult['PROPERTIES']['image']['VALUE'] as $key => $img):?>
                        <div class="card__img js-tab-item">
                            <div class="card-img-btns">
                                <button class="ui-btn zoom-link" href="<?=CFile::GetPath($img)?>">
                                    <img class="svg lazyload" data-src="/local/templates/fsk/img/icons/ic-zoom.svg" width="15" height="15" alt="zoom">
                                </button>
                                <div class="interactive-btn interactive-favorite" href="#" data-role="favorite" data-id="<?=$arResult['ID']?>">
                                    <img class="svg lazyload" data-src="/local/templates/fsk/img/icons/ic-star.svg" alt="Добавить в закладки">
                                </div>
                            </div>
                            <img class="img lazyload" data-src="<?=CFile::GetPath($img)?>" alt="plan">
                        </div>
                    <?endforeach?>
                </div>
                <div class="card__col-3">
                    <div class="card__info">
                        <div class="interactive-btns">
                            ...
                            <a class="interactive-btn interactive-print" href="print.html" target="_blank" title="Распечатать" data-id="<?=$arResult['ID']?>"></a>
                            <div class="interactive-btn interactive-favorite" data-role="favorite" data-id="<?=$arResult['ID']?>">
                                <img class="svg lazyload" data-src="/local/templates/fsk/img/icons/ic-star.svg" alt="Добавить в закладки">
                            </div>
                        </div>
                        <div class="card-data">
                            <div class="card-data__col-1">
                                <div class="h3 card-data__title"><?=$arResult['NAME']?> м<sup>2</sup></div>
                                <ul class="card-data__list">
                                    <?if($arResult['PROPERTIES']['area']['VALUE']):?>
                                        <li><span>Общая площадь</span><span><?=$arResult['PROPERTIES']['area']['VALUE']?> м<sup>2</sup></span></li>
                                    <?endif?>
                                    <?if($arResult['PROPERTIES']['livingspace']['VALUE']):?>
                                        <li><span>Жилая площадь</span><span><?=$arResult['PROPERTIES']['livingspace']['VALUE']?> м<sup>2</sup></span></li>
                                    <?endif?>
                                    <?if($arResult['PROPERTIES']['kitchenspace']['VALUE']):?>
                                        <li><span>Площадь кухни</span><span><?=$arResult['PROPERTIES']['kitchenspace']['VALUE']?> м<sup>2</sup></span></li>
                                    <?endif?>
                                    <?if($arResult['PROPERTIES']['floor']['VALUE']):?>
                                        <li><span>Этаж / этажей</span><span><?=$arResult['PROPERTIES']['floor']['VALUE']?>/<?=$arResult['PROPERTIES']['floorstotal']['VALUE']?></span></li>
                                    <?endif?>
                                    <?if($arResult['PROPERTIES']['renovation']['VALUE']):?>
                                        <li><span>Отделка</span><span><?=$arResult['PROPERTIES']['renovation']['VALUE']?></span></li>
                                    <?endif?>
                                    <?if($arResult['PROPERTIES']['buildingsection']['VALUE']):?>
                                        <li><span>Корпус</span><span><?=$arResult['PROPERTIES']['buildingsection']['VALUE']?></span></li>
                                    <?endif?>
                                    <?if($arResult['PROPERTIES']['section']['VALUE']):?>
                                        <li><span>Секция</span><span><?=$arResult['PROPERTIES']['section']['VALUE']?></span></li>
                                    <?endif?>
                                </ul>
                            </div>
                            <div class="card-data__col-2">
                                <?if($arResult['PROPERTIES']['price100']['VALUE']):?>
                                    <p class="card-price-1">Цена по акции: <span><?=number_format($arResult['PROPERTIES']['price100']['VALUE'], 0, '.', ' ')?></span></p>
                                <?endif;?>
                                <?if($arResult['PROPERTIES']['price']['VALUE']):?>
                                    <p class="card-price-2">Стандартная цена: <span><?=number_format($arResult['PROPERTIES']['price']['VALUE'], 0, '.', ' ')?></span></p>
                                <?endif;?>
                                <?if($arResult['XML_ID']):?>
                                    <span class="card-id">ID квартиры: <?=number_format($arResult['XML_ID'], 0, '.', ' ')?></span>
                                <?endif?>
                                <div class="card-btn">
                                    <a href="#modal-FORM10" class="popup-btn-FORM10 btn btn--bg js-call-callback" type="button">
                                        <svg class="svg btn__ic ic-tel" xmlns="http://www.w3.org/2000/svg" width="14.547" height="24.331" viewBox="0 0 14.547 24.331"> <g transform="translate(-8967.75 1214.75)"> <path d="M0,.5H13.047" transform="translate(8968.5 -1195.909)" fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="1.5"></path> <path d="M0,1.25H3.262" transform="translate(8973.393 -1211.988)" fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="1.5"></path> <rect width="13.047" height="22.831" rx="2" transform="translate(8968.5 -1214)" stroke-width="1.5" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" fill="none"></rect></g></svg>
                                        Консультация
                                    </a>
                                    <div data-type="reserved" class="btn--reserved">
                                        <svg class="svg btn__ic ic-tel" width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M7.5 10L7.5 12.6654" stroke="white" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M9.91016 13L4.82144 13" stroke="white" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> <rect x="0.75" y="0.75" width="13.5" height="8.5" rx="1.25" stroke="white" stroke-width="1.5"></rect></svg>
                                        Забронировано
                                    </div>
                                    <a href="/reserve/" data-type="reserveBtn" data-id="<?=$arResult['ID']?>" data-iblock="1" class=" btn btn--cta " type="button">
                                        <svg class="svg btn__ic ic-tel" width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M7.5 10L7.5 12.6654" stroke="white" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><path d="M9.91016 13L4.82144 13" stroke="white" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><rect x="0.75" y="0.75" width="13.5" height="8.5" rx="1.25" stroke="white" stroke-width="1.5"></rect></svg>
                                        Забронировать
                                    </a>
                                </div>
