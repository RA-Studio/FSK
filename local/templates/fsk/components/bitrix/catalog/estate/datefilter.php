...
/*                    $dateArr = []; // создаем массив дат
        ...
        <?if($_REQUEST['getVideo'] == "Y"):?>
            <?$APPLICATION->RestartBuffer();?><?=$ajaxGallery[0]['userProps']['UF_VIDEO']['VALUE'];die();?>
        <?endif;?>
        <div class="gallery-slider-xl" id="gallery-2">
            <?php foreach($ajaxGallery as $item) : ?>
                <?$date = $item['standartProps']['ACTIVE_FROM'] ? $item['standartProps']['ACTIVE_FROM'] : $progressGallery[$last]['standartProps']['TIMESTAMP_X'];?>
                    ...
                    <?php foreach ($item['userProps']['UF_GALLERY']['VALUE'] as $month) : ?>
                        <div class="slide">
                            ...
                            <button class="ui-btn zoom-link" href="<?=CFile::GetPath($month);?>" data-fancybox="gallery2">
                                ...
                                <div class="publish-date">Опубликовано: <?=FormatDate("d.m.Y",MakeTimeStamp($date))?><?=$item['userProps']['UF_RESPONSIBLE_NAME']['VALUE']?' - '.$item['userProps']['UF_RESPONSIBLE_NAME']['VALUE']:''?></div>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            <?php endforeach; ?>
